...
import { Company, Owner, Employee, Document, Plan } from '@shared/schema';
...
export function PdfFormGenerator({ companyId, carrier }: PdfFormGeneratorProps) {
  ...
  return (
    <div className="mt-4">
      <Button onClick={handleGenerateForm} disabled={isGenerating} variant="secondary">
        {isGenerating ? 'Generating...' : `Generate ${carrier} Application Form`}
      </Button>
      <p className="text-sm text-muted-foreground mt-2">
        This will generate a pre-populated PDF application form based on the data you've entered so
        far.
      </p>
    </div>
  );
