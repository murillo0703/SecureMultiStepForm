...
export default function PlanSelection() {
  ...
  const [selectedMetalTiers, setSelectedMetalTiers] = useState<string[]>(["Gold", "Silver", "Bronze", "Platinum"]);
          ...
          <div className="lg:flex-1">
            ...
            <Card className="mb-6">
              <CardHeader>
                ...
                <CardDescription>
                  Select health plans for your employees
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div className="mb-6">
                    ...
                    >
                      <div className={`border rounded-lg p-4 flex items-center cursor-pointer ${carrier === "Anthem" ? "border-primary bg-blue-50" : "hover:border-primary hover:bg-blue-50"}`}>
                        <RadioGroupItem value="Anthem" id="anthem" className="text-primary" />
                        <Label htmlFor="anthem" className="ml-3 cursor-pointer">
                          Anthem Blue Cross
                        </Label>
                      </div>
                      <div className={`border rounded-lg p-4 flex items-center cursor-pointer ${carrier === "Blue Shield" ? "border-primary bg-blue-50" : "hover:border-primary hover:bg-blue-50"}`}>
                        <RadioGroupItem value="Blue Shield" id="blueshield" className="text-primary" />
                        <Label htmlFor="blueshield" className="ml-3 cursor-pointer">
                          Blue Shield
                        </Label>
                      </div>
                      <div className={`border rounded-lg p-4 flex items-center cursor-pointer ${carrier === "CCSB" ? "border-primary bg-blue-50" : "hover:border-primary hover:bg-blue-50"}`}>
                    ...
                    <div className="border border-gray-200 rounded-lg divide-y">
                      {filteredPlans.length === 0 ? (
                        ...
                        filteredPlans.map(plan => (
                          <div key={plan.id} className="p-4 hover:bg-gray-50 flex flex-col sm:flex-row sm:items-center justify-between">
                            <div className="flex-1">
                              <div className="flex items-start">
                                ...
                                <div className="ml-3 text-sm">
                                  <label htmlFor={`plan-${plan.id}`} className="font-medium text-gray-700 cursor-pointer">
                                    {plan.name}
                                  </label>
                                  <p className="text-gray-500">{plan.details || `Network: ${plan.network}`}</p>
                                </div>
                              </div>
                            </div>
                            <div className="mt-2 sm:mt-0 flex flex-col sm:items-end text-sm">
                              <span className="font-medium text-gray-900">{plan.metalTier || plan.type}</span>
                          ...
                          <div key={plan.id} className="border border-green-200 bg-green-50 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-2">
                              <div>
                                ...
                                <p className="text-sm text-gray-600">{plan.metalTier ? `${plan.metalTier} tier` : plan.type}</p>
                              </div>
                              <button 
                                type="button" 
        ...
        <AlertDialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                You haven't selected any health plans. At least one plan selection is required to continue.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
