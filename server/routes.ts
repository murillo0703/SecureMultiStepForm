...
const isAuthenticated = (req: Request, res: Response, next: Function) => {
  console.log('Auth middleware - Session ID:', req.sessionID, 'Authenticated:', req.isAuthenticated(), 'User:', req.user?.username || 'none', 'Session:', !!req.session);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/application-initiator", isAuthenticated, validateRequest(insertApplicationInitiatorSchema), async (req, res, next) => {
    try {
  ...
  // Company routes
  app.post("/api/companies", isAuthenticated, validateRequest(insertCompanySchema), async (req, res, next) => {
    try {
      const userId = req.user!.id;
  ...
  // Owner routes
  app.post("/api/companies/:companyId/owners", isAuthenticated, validateRequest(insertOwnerSchema), async (req, res, next) => {
    try {
  ...
  // Employee routes
  app.post("/api/companies/:companyId/employees", isAuthenticated, validateRequest(insertEmployeeSchema), async (req, res, next) => {
    try {
      const companyId = parseInt(req.params.companyId);
      const company = await storage.getCompany(companyId);
  ...
  // Document routes
  app.post("/api/companies/:companyId/documents", isAuthenticated, upload.single("file"), async (req, res, next) => {
    try {
      const companyId = parseInt(req.params.companyId);
      const company = await storage.getCompany(companyId);
  ...
  app.post("/api/companies/:companyId/plans", isAuthenticated, validateRequest(insertCompanyPlanSchema), async (req, res, next) => {
    try {
      const companyId = parseInt(req.params.companyId);
      const company = await storage.getCompany(companyId);
  ...
  app.delete("/api/companies/:companyId/plans/:planId", isAuthenticated, async (req, res, next) => {
    try {
      ...
      await storage.createAuditLog({
        userId: req.user!.id,
        action: req.user!.role === "admin" ? AuditAction.ADMIN_PLAN_DELETE : AuditAction.APPLICATION_UPDATE,
  ...
  app.post("/api/companies/:companyId/contributions", isAuthenticated, validateRequest(insertContributionSchema), async (req, res, next) => {
    try {
      const companyId = parseInt(req.params.companyId);
      const company = await storage.getCompany(companyId);
  ...
  app.patch("/api/applications/:id", isAuthenticated, validateRequest(updateApplicationSchema), async (req, res, next) => {
    try {
  ...
  app.post("/api/admin/plans/upload", isAuthenticated, isAdmin, planFileUpload.single("planFile"), async (req, res, next) => {
    try {
      if (!req.file) {
        return res.status(400).json({ message: "No file uploaded" });
      }
  ...
  app.patch("/api/admin/users/:userId", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/admin/users/:userId/reset-password", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
      const userId = parseInt(req.params.userId);
  ...
  app.patch("/api/admin/brokers/:brokerId", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.patch("/api/admin/brokers/:brokerId/flag", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
      const brokerId = req.params.brokerId;
  ...
  app.get("/api/admin/pdf-templates", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/admin/pdf-templates/upload", isAuthenticated, isAdmin, pdfUpload.single('pdf'), async (req: Request, res: Response) => {
    try {
      if (!req.file) {
  ...
  app.post("/api/admin/pdf-templates/generate", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const mappings = pdfTemplates.default[carrierKey as keyof typeof pdfTemplates.default]?.fieldMappings || [];
  ...
  app.get("/api/admin/plan-uploads", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
      const uploadHistory = [
        {
          id: 1,
  ...
  app.get("/api/admin/audit-logs/export", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
      const logs = await storage.getRecentAuditLogs();
      const csv = "Timestamp,Action,User,Agency,Details,IP Address\n" +
        logs.map(log => `${log.timestamp},${log.action},User Name,Sample Agency,${log.details},192.168.1.1`).join("\n");
